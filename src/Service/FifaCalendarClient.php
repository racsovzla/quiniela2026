...
class FifaCalendarClient
{
     ...
     * @param array<string, mixed> $row
     */
    public function isPostponed(array $row): bool
    {
        ...
        if (is_int($matchStatus) && in_array($matchStatus, [3, 4, 5], true)) {
            return true;
        }

        foreach ($this->localizedDescriptions($row, 'MatchStatus') as $text) {
            $normalized = strtolower($text);
            if (str_contains($normalized, 'postpon')
                || str_contains($normalized, 'delay')
                || str_contains($normalized, 'retras')
            ) {
                return true;
            }
        }
     ...
     * @param array<string, mixed> $row
     */
    public function isSuspendedInPlay(array $row): bool
    {
        if ($this->isFinished($row)) {
            return false;
        }
        ...
        $matchStatus = $row['MatchStatus'] ?? null;
        if (is_int($matchStatus) && in_array($matchStatus, [6, 7], true)) {
            return true;
        }

        foreach ($this->localizedDescriptions($row, 'MatchStatus') as $text) {
            $normalized = strtolower($text);
            if (str_contains($normalized, 'suspend')
                || str_contains($normalized, 'abandon')
                || str_contains($normalized, 'interrupt')
            ) {
                return true;
            }
        }
     ...
     * @param array<string, mixed> $row
     */
    public function hasFutureKickoff(array $row, \DateTimeImmutable $nowUtc): bool
    {
        $kickoffUtc = $this->kickoffIso($row);
        if ($kickoffUtc === null) {
            return false;
        }
        ...
        return $kickoffAt > $nowUtc;
    }

    /**
     * @param array<string, mixed> $row
     */
    public function kickoffChanged(array $row, Fixture $fixture): bool
    {
        $kickoffUtc = $this->kickoffIso($row);
        if ($kickoffUtc === null) {
            return false;
        ...
        return $fixture->getKickoffAt()->format('Y-m-d H:i:s') !== $kickoffAt->format('Y-m-d H:i:s');
    }
     ...
     * @return list<string>
     */
    private function localizedDescriptions(array $row, string $field): array
    {
        $descriptions = $row[$field] ?? [];
        if (!is_array($descriptions)) {
            if (is_string($descriptions) && $descriptions !== '') {
                return [$descriptions];
            }
